...
export default function Navbar() {
  ...
  // Función para cerrar menú y subir al tope
  const handleNavClick = () => {
    setIsMobileOpen(false);
    window.scrollTo({ top: 0, behavior: 'instant' });
  };

  // Cada vez que cambie la URL, sube siempre
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'instant' });
  }, [location.pathname]);
  ...
  return (
    <>
      {/* Barra dorada: Se va al scrollear */}
      <div className="announcement-bar">
        <p><b>Beneficio Cuyo:</b> Entrega personalizada en Mendoza y San Luis. Envíos a todo el país.</p>
      </div>
      ...
      <nav className="navbar">
        <div className="navbar__container">
          ...
          {/* MENÚ MÓVIL (Drawer) */}
          <div className={`navbar__drawer ${isMobileOpen ? 'navbar__drawer--open' : ''}`}>
